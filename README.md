...
**入力は 1 tick 1 バイトです。** 例外は CHIP-8 だけで、16 キーあるので
1 tick に 2 バイト読みます (`inputBytes: 2`)。これはページが宣言する必要が
あり、推測してはいけません。1 バイトのゲストに上位バイトを混ぜて渡すと、
そのゲストは*次の*フレームでそれを入力として受け取り、結果としてキーが
1 フレームおきにしか届きません。押しっぱなしが押しっぱなしに見えなくなり、
`:key_held` でしか動かない操作 (rbTris の下キー) が効かなくなります。
