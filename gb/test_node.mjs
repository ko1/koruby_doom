// Headless check for the Game Boy page: same run.js the browser uses.
//   node test_node.mjs [frames] [new|interp|ruby]
import { Worker, isMainThread, workerData } from 'node:worker_threads';
import { readFileSync } from 'node:fs';
import { fileURLToPath } from 'node:url';

const W = 160, H = 144, FRAME = W * H * 4;
const HERE = fileURLToPath(new URL('.', import.meta.url));

if (!isMainThread) {
  const { mod, rom, src, argv, ctlBuf, fbBuf, palBuf } = workerData;
  const { runGuest } = await import('../run.js');
  await runGuest({ mod, rom, src, argv, mount: '/gb', romName: 'rom.gb', frameBytes: FRAME,
  ...
  const B = {
    new:    { wasm: 'gb.wasm' },
    interp: { wasm: '../koruby-interp.wasm', src: 'gb_web.rb', argv: ['koruby', '--plain', '/gb/gb_web.rb'] },
    ruby:   { wasm: '../ruby.wasm', src: 'gb_web.rb', argv: ['ruby', '/gb/gb_web.rb'] },
  }[kind];
  const mod = await WebAssembly.compile(readFileSync(HERE + B.wasm));
  const rom = readFileSync(HERE + 'rom.gb');
  ...
  for (let p = 0; p < FRAME; p += 4) colours.add(fb[p] << 16 | fb[p+1] << 8 | fb[p+2]);
